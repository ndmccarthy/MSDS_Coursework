...
class BinarySearchTree:
    ...
    def inorderTraversal(self, node):
        # returns a list of the keys in the tree in order from smallest to largest
        # need to input root node by calling tree.root
        traversal = []
        if node is not None:
    ...
    def transplant(self, node_removing, child):
        # inputs are references to nodes, not just the keys
